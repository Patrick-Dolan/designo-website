    ...
    <>
      <nav className={`fixed w-full bg-white z-30 ${show ? 'fixed' : 'hidden'}`}>
        <div className="flex justify-between items-center mx-6 min-h-24 md:mx-10 md:py-16 lg:mx-41">
          ...
          <button onClick={mobileNavOpen ? closeMobileNav : openMobileNav} className="md:hidden">
            {mobileNavOpen ? <CloseIcon /> : <HamburgerIcon />}
          </button>
          <NavLinks className="hidden md:block" mobileNavOpen={mobileNavOpen} closeMobileNav={closeMobileNav} />
        </div>
        <NavLinks className="md:hidden" mobileNavOpen={mobileNavOpen} closeMobileNav={closeMobileNav} />
      </nav>
